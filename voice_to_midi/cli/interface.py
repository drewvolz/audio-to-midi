...
class CLIInterface:
    ...
    def select_audio_device(self, devices: List[AudioDevice], 
                           default_index: Optional[int] = None) -> Optional[AudioDevice]:
        """
        ...
        if default_index is not None and 0 <= default_index < len(choices):
            ...
            default_choice = next(i for i, device in enumerate(devices) if device.is_default)
    ...
    def select_midi_port(self, ports: List[MidiPort], 
                        default_index: Optional[int] = None) -> Optional[MidiPort]:
        """
    ...
    def configure_audio_settings(self, current_settings: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        sample_rate = questionary.select(
            ...
            default=f"{settings.get('sample_rate', 44100)} Hz"
        ).ask()
        ...
        chunk_size = questionary.select(
            ...
            default=f"{settings.get('chunk_size', 1024)} samples"
        ).ask()
    ...
    def configure_midi_settings(self, current_settings: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        channel = questionary.text(
            ...
            validate=lambda val: 0 <= int(val) <= 15
        ).ask()
        ...
        velocity = questionary.text(
            ...
            validate=lambda val: 1 <= int(val) <= 127
        ).ask()
        ...
        transpose = questionary.text(
            ...
            validate=lambda val: -24 <= int(val) <= 24
        ).ask()
        ...
        max_note = questionary.text(
            ...
            validate=lambda val: 0 <= int(val) <= 127
        ).ask()
    ...
    def configure_pitch_settings(self, current_settings: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def display_configuration_summary(self, settings: Dict[str, Any]) -> None:
        ...
        if 'audio' in settings:
            ...
            panel_content.append(f"  Chunk Size: {audio.get('chunk_size', 'N/A')} samples")
            panel_content.append(f"  Input Device: {audio.get('input_device_name', 'N/A')}")
        ...
        if 'midi' in settings:
            ...
            panel_content.append(f"  Output Port: {midi.get('output_port_name', 'N/A')}")
            panel_content.append(f"  Channel: {midi.get('channel', 'N/A')}")
            panel_content.append(f"  Velocity: {midi.get('velocity', 'N/A')}")
            panel_content.append(f"  Transpose: {midi.get('transpose_semitones', 'N/A')} semitones")
            panel_content.append("")
        ...
        if 'pitch' in settings:
            ...
            panel_content.append(f"  Frequency Range: {pitch.get('min_freq', 'N/A')}-{pitch.get('max_freq', 'N/A')} Hz")
            panel_content.append(f"  Confidence Threshold: {pitch.get('confidence_threshold', 'N/A')}")
        
        if panel_content:
            content = "\n".join(panel_content)
            panel = Panel(content, title="[bold]Current Configuration[/bold]", border_style="green")
    ...
    def display_pedal_learning_prompt(self) -> None:
        ...
        content = "\n".join(panel_content)
        panel = Panel(content, title="[bold]MIDI Pedal Configuration[/bold]", border_style="cyan")
        self.console.print(panel)
    
    def display_status(self, frequency: float, note_name: str, midi_note: int, 
        ...
        status_text = (
            ...
            f"[bold green]Confidence:[/bold green] {confidence:.2f}"
        )
        
        # Clear previous line and print new status
        self.console.print(f"\r{status_text}", end="")
    ...
    def display_error_panel(self, error: Exception) -> None:
        ...
        if isinstance(error, VoiceToMidiError):
            ...
            error_content.append("[yellow]Please check your configuration and try again.[/yellow]")
    ...
    def display_help_panel(self) -> None:
        ...
        content = "\n".join(help_content)
        panel = Panel(content, title="[bold]Voice to MIDI Help[/bold]", border_style="blue")
        self.console.print(panel)