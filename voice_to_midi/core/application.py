...
from ..core.exceptions import VoiceToMidiError, AudioError, MidiError
...
class VoiceToMidiApp:
    ...
    def configure_devices(self, force_selection: bool = False) -> None:
        ...
        if not self.settings:
            raise VoiceToMidiError("Configuration must be loaded before configuring devices")
        
        if not self.audio_device_manager or not self.midi_device_manager:
            raise VoiceToMidiError("Device managers must be injected before configuring devices")
    ...
    def start(self) -> None:
        ...
        if not all([self.audio_capture, self.audio_processor, self.pitch_detector, self.midi_output]):
        ...
        try:
            while self.is_running:
                try:
                    ...
                    # Detect pitch
                    frequency, confidence = self.pitch_detector.detect_pitch(processed_data)
                    ...
                    if frequency and confidence > self.settings.pitch.confidence_threshold:
        ...
        try:
            while self.is_running:
                try:
                    ...
                    if note is not None:
                        # Send note on
                        self.midi_output.send_note_on(note)
                    else:
                        # Send note off for current note
                        self.midi_output.send_all_notes_off()
    ...
    def is_configured(self) -> bool:
        ...
        return (
            ...
            self.settings.audio.input_device_name is not None and
            self.settings.midi.output_port_name is not None
        )