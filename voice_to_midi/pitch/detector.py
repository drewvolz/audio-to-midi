...
class PitchDetector:
    ...
    def configure(
        ...
        logger.info(
            f"Pitch detector configured: {min_freq}-{max_freq}Hz, threshold={confidence_threshold}"
        )
    ...
    def _apply_octave_correction(
        self, frequency: float, audio_data: np.ndarray
    ) -> float:
        """
    ...
    def _apply_smoothing(self, frequency: float) -> float:
        ...
        smoothed = self.last_frequency * self.smoothing_factor + frequency * (
    ...
    def analyze_harmonics(
        self, audio_data: np.ndarray, fundamental: float
