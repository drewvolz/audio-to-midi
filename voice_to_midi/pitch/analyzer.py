...
from ..utils.helpers import frequency_to_note_name
...
class PitchAnalyzer:
    ...
    def detect_vibrato(self, window_size: int = 50) -> Tuple[bool, float, float]:
        ...
        """
        if (
            len(self.pitch_history) < window_size
            or len(self.time_history) < window_size
        ):
        ...
        try:
            ...
            vibrato_mask = (freqs >= self.vibrato_min_rate) & (
                freqs <= self.vibrato_max_rate
            )
            ...
            has_vibrato = (
                vibrato_depth >= self.vibrato_min_depth
