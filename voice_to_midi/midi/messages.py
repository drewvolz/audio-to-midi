...
class MidiMessageHandler:
    ...
    def create_note_on(self, note: int, velocity: int, channel: int = 0) -> mido.Message:
        """
        ...
        try:
            ...
            logger.debug(f"Created note on: note={note}, velocity={velocity}, channel={channel}")
            return message
        except Exception as e:
            raise MidiError(f"Failed to create note on message: {e}")
    
    def create_note_off(self, note: int, velocity: int = 0, channel: int = 0) -> mido.Message:
        """
        ...
        try:
            message = mido.Message('note_off', note=note, velocity=velocity, channel=channel)
            logger.debug(f"Created note off: note={note}, velocity={velocity}, channel={channel}")
            return message
        except Exception as e:
            raise MidiError(f"Failed to create note off message: {e}")
    
    def create_control_change(self, control: int, value: int, channel: int = 0) -> mido.Message:
        """
        ...
        try:
            message = mido.Message('control_change', control=control, value=value, channel=channel)
            logger.debug(f"Created control change: control={control}, value={value}, channel={channel}")
            return message
        except Exception as e:
    ...
    def create_program_change(self, program: int, channel: int = 0) -> mido.Message:
        ...
        try:
            ...
            logger.debug(f"Created program change: program={program}, channel={channel}")
            return message
        except Exception as e:
    ...
    def validate_message(self, message: mido.Message) -> bool:
        ...
        try:
            ...
            if message.type not in ['note_on', 'note_off', 'control_change', 
                                   'pitchwheel', 'program_change', 'aftertouch', 'sysex']:
            ...
            if message.type in ['note_on', 'note_off']:
                ...
                self._validate_velocity(message.velocity, allow_zero=(message.type == 'note_off'))
    ...
    def message_to_dict(self, message: mido.Message) -> Dict[str, Any]:
        ...
        for attr in ['note', 'velocity', 'control', 'value', 'pitch', 'program', 'channel']:
